...
class MDHDialog(QDialog):
    ...
    def create_left_panel(self):
        ...
        self.mdh_table.setHorizontalHeaderLabels(
            ["Joint", "θ (rad)", "d", "a", "α (rad)"]
        )
    ...
    def create_right_panel(self):
        ...
        self.header_text_edit.setPlaceholderText(
            "Header file or Python usage content will be displayed here..."
        )
        ...
        self.cpp_text_edit.setPlaceholderText(
            "Source file content will be displayed here..."
        )
        # Set monospace font for code
        font = QFont("Consolas", 10)
        font.setStyleHint(QFont.Monospace)
        ...
        if file_path:
            try:
                with open(file_path, "w") as f:
                    ...
                    f.write(
                        f"{'Joint':<10} {'theta (rad)':<12} {'d':<12} {'a':<12} {'α (rad)':<12}\n"
                    )
                    f.write("-" * 60 + "\n")
                    ...
                    for i, params in enumerate(self.mdh_parameters):
                        ...
                        f.write(
                            f"{joint_name:<10} {theta:<4.8f} {d:<4.8f} {a:<4.8f} {alpha:<4.8f}\n"
                        )
                ...
                QMessageBox.information(
                    self, "Success", f"MDH parameters saved to:\n{file_path}"
                )
            except Exception as e:
                QMessageBox.critical(
                    self, "Error", f"Failed to save MDH parameters:\n{str(e)}"
                )
